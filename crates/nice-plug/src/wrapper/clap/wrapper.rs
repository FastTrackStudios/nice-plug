...
impl<P: ClapPlugin> Wrapper<P> {
    ...
    /// Whether the plugin supplied an embedded (REAPER inline FX UI) editor.
    ///
    /// The embedded editor lives behind the `editor` feature, so this collapses
    /// to `false` when that feature is off and the extension is never
    /// advertised.
    fn has_embedded_editor(wrapper: &Self) -> bool {
        #[cfg(feature = "editor")]
        {
            wrapper.embedded_editor.borrow().is_some()
        }
    ...
    unsafe extern "C" fn get_extension(
        ...
        } else if id == CLAP_EXT_REAPER_EMBED_UI && Self::has_embedded_editor(wrapper) {
            // Only advertise REAPER's embedded UI extension when the plugin
            // actually has an embedded editor.
