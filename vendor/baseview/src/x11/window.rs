    ...
    fn window_handle(&self) -> Result<RwhWindowHandle<'_>, HandleError> {
        if let Some(raw_window_handle) = self.raw_window_handle {
            if self.is_open.load(Ordering::Relaxed) {
                // Safety: The raw handle is valid while the window is open
                return Ok(unsafe { RwhWindowHandle::borrow_raw(raw_window_handle) });
            }
        }

        // Return an empty/dummy handle when window is closed
...
impl<'a> Window<'a> {
        ...
        B: Send + 'static,
    {
        ...
        let parent_handle = parent.window_handle().expect("Failed to get parent window handle");
        ...
        B: Send + 'static,
    {
        ...
        let raw_handle = window.window_handle().expect("Failed to get window handle").as_raw();
        let _ = tx.send(Ok(SendableRwh(raw_handle)));
    ...
    pub fn size(&self) -> (u32, u32) {
        let phys = self.inner.window_info.physical_size();
...
impl<'a> HasWindowHandle for Window<'a> {
    fn window_handle(&self) -> Result<RwhWindowHandle<'_>, HandleError> {
        ...
        handle.visual_id = self.inner.visual_id as c_ulong;
...
impl<'a> HasDisplayHandle for Window<'a> {
    fn display_handle(&self) -> Result<DisplayHandle<'_>, HandleError> {
        ...
        let screen = unsafe { x11::xlib::XDefaultScreen(display) } as c_int;
