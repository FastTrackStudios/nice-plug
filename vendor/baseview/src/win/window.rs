...
impl HasWindowHandle for WindowHandle {
    fn window_handle(&self) -> Result<RwhWindowHandle<'_>, HandleError> {
        if let Some(hwnd) = self.hwnd {
            ...
            let handle = Win32WindowHandle::new(std::num::NonZeroIsize::new(1).unwrap());
            Ok(unsafe { RwhWindowHandle::borrow_raw(RawWindowHandle::Win32(handle)) })
        }
    }
}
...
impl Window<'_> {
        ...
        B: Send + 'static,
    {
        ...
        let parent = match parent_handle.as_raw() {
    ...
    pub fn size(&self) -> (u32, u32) {
        ...
        let phys = window_info.physical_size();
...
impl HasWindowHandle for Window<'_> {
    fn window_handle(&self) -> Result<RwhWindowHandle<'_>, HandleError> {
        let handle = Win32WindowHandle::new(
            std::num::NonZeroIsize::new(self.state.hwnd as isize).unwrap()
        );
        Ok(unsafe { RwhWindowHandle::borrow_raw(RawWindowHandle::Win32(handle)) })
    }
}
