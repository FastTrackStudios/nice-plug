...
impl HasWindowHandle for WindowHandle {
    fn window_handle(&self) -> Result<RwhWindowHandle<'_>, HandleError> {
        let raw = self.state.window_inner.raw_window_handle();
...
impl HasDisplayHandle for WindowHandle {
    fn display_handle(&self) -> Result<DisplayHandle<'_>, HandleError> {
        ...
        // SAFETY: macOS display handles don't contain borrowed data
    ...
    fn raw_window_handle(&self) -> RawWindowHandle {
        if self.open.get() {
            let ns_view = NonNull::new(self.ns_view as *mut c_void)
                .expect("ns_view should not be null");
            let handle = AppKitWindowHandle::new(ns_view);
            return RawWindowHandle::AppKit(handle);
        }

        // Return a dummy handle when window is closed
        let dummy = NonNull::new(1 as *mut c_void).unwrap();
        RawWindowHandle::AppKit(AppKitWindowHandle::new(dummy))
    }
}
...
impl<'a> Window<'a> {
        ...
        B: Send + 'static,
    {
        ...
        let raw_handle = parent.window_handle().expect("Failed to get parent window handle").as_raw();
        let handle = if let RawWindowHandle::AppKit(handle) = raw_handle {
            handle
        } else {
    ...
    /// Get the current size of the window in physical pixels
    pub fn size(&self) -> (u32, u32) {
        if self.inner.open.get() {
            unsafe {
                let bounds: NSRect = msg_send![self.inner.ns_view, bounds];
                // Get the backing scale factor to convert to physical pixels
...
impl<'a> HasWindowHandle for Window<'a> {
    fn window_handle(&self) -> Result<RwhWindowHandle<'_>, HandleError> {
        let raw = self.inner.raw_window_handle();
        // SAFETY: The window handle is valid for the lifetime of self
        Ok(unsafe { RwhWindowHandle::borrow_raw(raw) })
    }
}
